...
def vecCheck(df):
    # df = df.copy()
    # df = APNsetup(df) # get APN with no dash and uniqueAPNs left alone.
    df = df[['APN', 'Street #', 'Street Name', 'Debris Finish', 'Number of Vehicles',
             'Number of Vehicles Removed', 'County']]
    df = df.set_index("APN")
    todaysDate = time.strftime("%m-%d-%y")
...
# TODO: getTotalsGraph(df) ------>work on it not ready
#  Graph out sheet->get graph to only count numbers, and number >0 ONLY!
# def getTotalsGraph(df):  # this function is for getting counts by any way you need.
...
# getTotalsGraph(df) # test function
